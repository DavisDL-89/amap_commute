...
class AmapCommuteCoordinator(DataUpdateCoordinator):
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        ...
        self.use_mobile_origin = (
            entry.data.get(CONF_ORIGIN_MODE) == ORIGIN_MODE_MOBILE
            or entry.data.get(CONF_USE_MOBILE_ORIGIN, False)
        )
        self.mobile_tracker_entity = entry.data.get(CONF_MOBILE_TRACKER_ENTITY, "")
    ...
    async def _fetch_route_data(self) -> dict:
        ...
        if data.get("status") != "1":
            ...
            raise UpdateFailed(
                f"高德API错误: {self._get_api_error_message(info, infocode)} (code: {infocode})"
            )
        ...
        for idx, path in enumerate(paths[:3]):
            ...
            route_details = self._parse_route_details(path)
            ...
            _LOGGER.debug("路线 %d 标签: %s", idx, route_label)
            ...
            all_routes.append({
                ...
                "tmcs":             route_details["tmcs"],
                "steps":            route_details["steps"],
            })

        # 默认展示第一条路线的关键字段（保持向后兼容）
        best = all_routes[0]
        current_origin, current_origin_name = self._get_origin_meta()
        return {
            # ── 当前路线关键字段（向后兼容） ──
            ...
            "mobile_origin_unavailable": self.use_mobile_origin and current_origin == self.origin,
    ...
    def _get_current_origin(self) -> str:
        """获取当前出发地坐标（支持手机定位实时更新）."""
        origin, _ = self._get_origin_meta()
        return origin

    def _get_origin_meta(self) -> tuple[str, str]:
        """返回 (origin_coord, origin_name)."""
        if self.use_mobile_origin and self.mobile_tracker_entity:
            state = self.hass.states.get(self.mobile_tracker_entity)
            if state is not None:
                lat = state.attributes.get("latitude")
                lon = state.attributes.get("longitude")
                if lat is not None and lon is not None:
                    origin_coord = f"{float(lon):.6f},{float(lat):.6f}"
                    mobile_name = state.attributes.get("friendly_name") or "手机定位"
                    return origin_coord, mobile_name
            return self.origin, "无法获取手机定位"
        return self.origin, self.origin_name

    def _parse_route_details(self, path: dict) -> dict:
        """解析单条路线的折线、路况分段和步骤信息."""
        polylines: list = []
        tmcs: list = []
        steps_data: list = []
                ...
                tmcs.append(
                    {
                        ...
                        "polyline": self._parse_polyline(tmc_polyline) if tmc_polyline else [],
                    }
                )
        ...
        return {"polyline": polylines, "tmcs": tmcs, "steps": steps_data}

    @staticmethod
    def _get_api_error_message(info: str, infocode: str) -> str:
        """把高德 infocode 映射为更可读的错误提示."""
        infocode_messages = {
            "10001": "无效的 API Key",
            "10002": "服务未开通或无权限",
            "10003": "访问已超出调用配额",
        ...
        return infocode_messages.get(infocode, info or "未知错误")

...
async def async_setup_entry(
    ...
    # 首次刷新已在 integration async_setup_entry 中完成，此处直接读 coordinator.data
...
class AmapCommuteSensor(CoordinatorEntity, SensorEntity):
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        ...
        if data.get(ATTR_ORIGIN_MODE) is not None:
            attrs[ATTR_ORIGIN_MODE] = data.get(ATTR_ORIGIN_MODE)
        if data.get(ATTR_ORIGIN_ENTITY):
            attrs[ATTR_ORIGIN_ENTITY] = data.get(ATTR_ORIGIN_ENTITY)
        return attrs
